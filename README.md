# 루나소프트 사내홈페이지 (촬영용 소품 화면)

〈쩔해주세요〉 촬영용 가짜 사내 그룹웨어 화면. 화면 설정(셋업)을 서버(Upstash Redis)에 이름별로 저장해 여러 PC에서 공유합니다.
...
- 단축키: `H` 안내 표시, `S` 설정 메뉴, `F`/`T` 글 자동 입력, `Enter` 등록, `C` 댓글 폭주, `P` 인사발령 팝업, `M` 메신저 알림, `1/2/3` 화면 이동, `R` 초기화
